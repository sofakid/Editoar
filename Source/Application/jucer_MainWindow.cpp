...
MainWindow::MainWindow()
    : DocumentWindow (EditoarApplication::getApp().getApplicationName(),
                      ...
                      false)
{
   ...
   #if ! JUCE_MAC
    setMenuBar (EditoarApplication::getApp().getMenuModel());
   #endif
    ...
    ApplicationCommandManager& commandManager = EditoarApplication::getCommandManager();
...
MainWindow::~MainWindow()
{
    ...
    removeKeyListener (EditoarApplication::getCommandManager().getKeyMappings());
...
{
    EditoarApplication::getApp().mainWindowList.closeWindow (this);
}
...
bool MainWindow::closeProject (Project* project)
{
    ...
    if (! EditoarApplication::getApp().openDocumentManager.closeAllDocumentsUsingProject (*project, true))
        return false;
...
bool MainWindow::canOpenFile (const File& file) const
{
    return (! file.isDirectory())
             && (file.hasFileExtension (Project::projectFileExtension)
                  || EditoarApplication::getApp().openDocumentManager.canOpenFile (file));
}
...
void MainWindow::activeWindowStatusChanged()
{
    ...
    EditoarApplication::getApp().openDocumentManager.reloadModifiedFiles();
    ...
    if (Project* p = getProject())
    {
        if (p->hasProjectBeenModified())
        {
            ...
            {
                EditoarApplication::getApp().getCommandManager().invokeDirectly (CommandIDs::saveProject, true);
            }
        }
    }
}
...
{
    String name (EditoarApplication::getApp().getApplicationName());
