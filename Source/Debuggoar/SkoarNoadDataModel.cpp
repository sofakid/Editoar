...
// --- SkoarNoaditeDataModel ----------------------------------------------------------
SkoarNoaditeDataModel::SkoarNoaditeDataModel (const SkoarNoadite& p) :
    name (p.name.c_str ()),
    skoarpuscle (p.skoarpuscle == nullptr ? L"" : p.skoarpuscle->asString ().c_str ()),
    voice (p.voice == nullptr ? L"" : p.voice->name.c_str ()),
    offs (p.offs),
...
cause it to refresh the list.
*/
int SkoarNoaditeDataModel::getNumRows () {
    return names_n;
}
...
list, so be careful that you don't assume it's less than getNumRows().
*/
void SkoarNoaditeDataModel::paintRowBackground (Graphics& g,
    int /*rowNumber*/,
    int width, int height,
    bool rowIsSelected) {
...
list, so be careful that you don't assume it's less than getNumRows().
*/
void SkoarNoaditeDataModel::paintCell (Graphics& g,
    int rowNumber,
    int columnId,
    int width, int height,
    bool /*rowIsSelected*/) {
...
This is used by TableListBox::autoSizeColumn() and TableListBox::autoSizeAllColumns().
*/
int SkoarNoaditeDataModel::getColumnAutoSizeWidth (int columnId) {
    const int char_width = 8;
    if (columnId == EColumn::field)
    {
        return String ("skoarpuscle: ").length () * char_width;
