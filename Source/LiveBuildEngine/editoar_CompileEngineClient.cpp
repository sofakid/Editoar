...
#include "editoar_CompileEngineClient.h"
#include "../LiveBuildEngine/editoar_CompileEngineServer.h"
...
namespace ProjectProperties
{
    ...
    static File getEditoarTempFolder()
    {
       #if JUCE_MAC
        return File ("~/Library/Caches/org.skoarcery.editoar");
       #else
        return File::getSpecialLocation (File::tempDirectory).getChildFile ("org.skoarcery.editoar");
       #endif
    }
...
void CompileEngineChildProcess::openPreview (const ClassDatabase::Class& comp)
{
    if (process != nullptr)
    {
        ...
        OwnedArray<MainWindow>& windows = EditoarApplication::getApp().mainWindowList.windows;
...
{
    for (MainWindow* mw : EditoarApplication::getApp().mainWindowList.windows)
