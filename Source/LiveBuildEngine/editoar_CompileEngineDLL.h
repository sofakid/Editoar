...
#include "editoar_LiveCodeBuilderDLL.h"
...
struct CompileEngineDLL
{
    ...
    {
        if (isLoaded())
            editoar_initialise (crashFn, quitFn, setPropertyCallback, getPropertyCallback, setupSignals);
    }
    ...
    bool isLoaded() const
    {
        ...
        return editoar_getVersion() == requiredVersion;
    }
    ...
    static File getVersionedUserAppSupportFolder()
    {
        ...
        return userAppData.getChildFile (String ("Editoar-") + ProjectInfo::versionString);
    }
